  ...
  app.get('/jomla/product/:slug', async (req, res) => {
    try {
      ...
      const galleryHtml = images.map((img, idx) => `
        <img src="${base}/jomla/product-image/${img.id}" alt="${title}" ${idx === 0 ? '' : 'loading="lazy"'} class="gallery-img">
      `).join('');
...
<link rel="stylesheet" href="/jomla/jomla.css">
<script type="application/ld+json">${jsonLd}</script>
</head>
<body>
<header class="store-header">
  <a href="/jomla" class="store-logo"><img src="/images/drrose-logo.png" alt="دورو"></a>
  <a href="/jomla/cart.html" class="cart-link">السلة 🛒</a>
</header>
